...
%%--------------------------------------------------
%%
       if (perturb_C) && (theta > 1e-10) %%2nd condition added: 2017-Jun-13
...
%%--------------------------------------------------
      if (theta > 1e-10) %%added: 2017-Jun-13
         perturb_Z = 1;  
      else
         perturb_Z = 0; %% do not perturb when theta is small
      end
...
%%--------------------------------------------------
      if ((prim_infeas < 1.5*prim_infeas_best) ...                
         | (max(relgap*0.1,infeas) < 0.8*max(relgap_best,infeas_best))) ...
         & (max(relgap*0.1,dual_infeas) < 0.8*max(relgap_best,dual_infeas_best)) 
         %% ()*1 -> ()*0.1: 2017-Jun-13
         Xbest = X; ybest = y; Zbest = Z; 
         kapbest = kap; taubest = tau; thetabest = theta; 
         prim_infeas_best = prim_infeas; 
