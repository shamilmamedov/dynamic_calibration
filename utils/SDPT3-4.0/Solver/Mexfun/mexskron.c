...
***********************************************************/
void skron(mwSize n, mwSize maxblksize, double *P, double *Q,
           double *x1, double *y1, double *x2, double *y2,
           mwSize r, mwSize c, double *vvtmp)
...
* A=B 
***********************************************************/
void skron2(mwSize n, mwSize maxblksize, double *P, double *Q,
            double *x1, double *y1, double *x2, double *y2,
            mwSize r, mwSize c, double *vvtmp)
...
{    mxArray  *blk_cell_pr;
     ...
     mwIndex  *blksize, *blksize2, *cumblksize, *blksize4;
     mwSize    isspA, isspB, sym;

     mwIndex   subs[2];
     mwSize    nsubs=2; 
     mwSize    m, n, n2, nsub, k, index, numblk, len, maxblksize; 
     mwSize    i, j, l, jn, idxstart, idxend, kstart, kend, idx, blklen; 
     mwSize    blklen2, rowidx, colidx; 
...
/* CHECK THE DIMENSIONS */
    ...
    blksize    = mxCalloc(numblk,sizeof(mwSize));
    for (k=0; k<numblk; k++) { blksize[k] = (mwSize) blksizetmp[k]; }
    cumblksize = mxCalloc(numblk+1,sizeof(mwSize));
    blksize2   = mxCalloc(numblk+1,sizeof(mwSize)); 
    blksize4   = mxCalloc(numblk+1,sizeof(mwSize)); 
