...
void ops1(double *x, double *y, double *z, 
          mwSize numblk, mwIndex *cumblk, mwSize options) 
...
/* CHECK THE DIMENSIONS */
    ...
    cumblk = mxCalloc(numblk+1,sizeof(mwSize)); 
    for (l=0; l<numblk; l++) { 
        cols = (mwSize)blksize[l]; 
        cumblk[l+1] = cumblk[l] + cols;  
    } 
    n = cumblk[numblk]; 
    /***** assign pointers *****/
    options = (mwSize)*mxGetPr(prhs[3]); 
    if (mxGetM(prhs[2]) != n) {
